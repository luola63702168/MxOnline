...
from users.views import LoginView, RegisterView, ActivateUserView, ForgetPwdView, ResetView, ModifyPwdView, \
    UserInfoView, LogoutView,UploadImageView,UpdatePwdView
...
urlpatterns = [
    ...
    url(r'^info/$', UserInfoView.as_view(), name="user_info"),  # 用户信息页
    ...
    url(r'^sendemail_code/$', LogoutView.as_view(), name='sendemail_code'),  # 修改邮箱时 发送邮箱验证码
    url(r'^update_email/$', LogoutView.as_view(), name='update_email'),  # 修改邮箱时，验证邮箱和验证码
    url(r'^mycourse/$', LogoutView.as_view(), name='mycourse'),  # 我的课程
    url(r'^myfav/org/$', LogoutView.as_view(), name='myfav_org'),  # 我收藏的课程机构
    url(r'^myfav/teacher/$', LogoutView.as_view(), name='myfav_teacher'),  # 我收藏的授课讲师
    url(r'^myfav/course/$', LogoutView.as_view(), name='myfav_course'),  # 我收藏的课程
    url(r'^mymessage/$', LogoutView.as_view(), name='mymessage'),  # 我的消息
